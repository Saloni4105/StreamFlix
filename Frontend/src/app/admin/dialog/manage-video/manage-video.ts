...
import { RATINGS, VIDEO_CATEGORIES } from '../../../shared/constants/app.constant';
...
export class ManageVideo implements OnInit {
  ...
  onVideoPicked(eve: Event) {
    const file = (eve.target as HTMLInputElement).files?.[0];
    if (!file) return;
